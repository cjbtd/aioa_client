...
* @Time    2022-2-26 JS to TS, add type declaration in fn arguments by Dawn
* @Version 1.0.3
...
const calendar = {
  ...
  toAstro: function (cMonth: number, cDay: number) {
    ...
    var index = cMonth * 2 - (cDay < arr[cMonth - 1] ? 2 : 0)
    return s.substring(index, index + 2) + "\u5ea7";//座
  },
  ...
  solar2lunar: function (y: number, m: number, d: number) { //参数区间1900.1.31~2100.12.31
    ...
    //未传参  获得当天
    var objDate = y ? new Date(y, m - 1, d) : new Date();
